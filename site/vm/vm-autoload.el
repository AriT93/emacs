...
The digests messages are assimilated into the folder as new mail
would be.
...
The digests messages are copied to a buffer and vm-mode is
...
whenever this expressions value is non-nil.
...
The first element should be the submenu name.  Thats used as the
...
message.  If you dont want your edited version of the message to
...
vm-message-list, so this slot must be valid in END-POINTs message
...
Also mark Ms buffer as needing a display update. Any virtual
...
M (i.e. attribute-of eq attributes-of Ms real message), Ms real
message and its buffer are scheduled for an update.

Optional arg DONT-KILL-CACHE non-nil means dont invalidate the
...
folder buffers summary and presentation buffers, and then
...
vm-text-of and vm-vheaders-of fields dont get filled until they
are needed.
...
Prefix arg is handled the same as for the command save-buffer.
Expunge wont be done if folder is read-only.
...
call-interactive cant be used unfortunately, but this-command is
...
COMPOSITIONs name will be read from the minibuffer." t nil)
...
(autoload (quote vm-keyboard-read-file-name) "vm-minibuf" "Like read-file-name, except HISTORYs value is unaltered." nil nil)
...
(autoload (quote vm-get-file-buffer) "vm-misc" "Like get-file-buffer, but also checks buffers against FILEs truename" nil nil)
...
(autoload (quote vm-mouse-send-url-to-safari) "vm-mouse" "Sends URL to Safari, using Apples Open Scripting Architecture." nil nil)
...
Dont call this function from a program." t nil)
...
are replying to, forwarding, or invoked VMs mail command from.
...
for you to fill in the new recipient list.  If you dont fill in
...
(autoload (quote vm-isearch-forward) "vm-search" "Incrementally search forward through the current folders messages.
...
(autoload (quote vm-isearch-backward) "vm-search" "Incrementally search backward through the current folders messages.
Usage is identical to the standard Emacs incremental search.
When the search terminates the message containing point will be selected.
...
All the messages can be read by repeatedly pressing SPC.  Use `next and
`previous to move about in the folder.  Messages are marked for
...
Commands:
 ...
 M N - use marks; the next vm command will affect only marked messages
   ...
   V - prefix for virtual folder commands:
       ...
       V M - toggle whether this virtual folders messages mirror the
 ...
 C-_ - undo, special undo that retracts the most recent
   ...
   $ - prefix for MIME commands.  Position the cursor over a MIME
       ...
       $ RET - display the MIME objects text using the \"default\" face.
 ...
 M-W - view the details of VMs lack of a warranty
