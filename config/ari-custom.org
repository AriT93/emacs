...
** org-remark notes file naming
org-remark's default ~org-remark-notes-file-name~ is the literal string
~"marginalia.org"~, which means every file annotated in the same
directory shares one marginalia file -- fine for one file at a time, but
it makes it impossible to tell which highlight belongs to which source
once more than one file in a directory has been annotated, and switching
between them clobbers the notes buffer. org-remark's built-in function
option (~org-remark-notes-file-name-function~) already fixes the
one-file-per-directory problem by deriving ~FILE-notes.org~ from the
source file name, but keeps the source name's own casing/spacing as-is.
This variant additionally snake_cases the source's base name before
appending ~_notes.org~, so e.g. =My Cool File.txt= gets
=my_cool_file_notes.org= -- consistent, shell/link-friendly notes file
names, and several files can be open and annotated in the same buffer
session at once, each keeping its own distinct notes file.
